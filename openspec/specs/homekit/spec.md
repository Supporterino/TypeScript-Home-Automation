...
- **THEN** it invokes the factory with a single `HomekitServiceContext` object
  containing `http`, `logger`, `mqtt`, `deviceRegistry`, and `shelly`
...
bridge MAY still start to serve the Shelly source. If no source is available, the
service MUST log a warning and skip startup.
